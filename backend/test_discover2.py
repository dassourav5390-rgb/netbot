...
with httpx.Client(verify=False, timeout=15) as client:
    ...
    for oid in oids_to_try:
        try:
            result = cgi_get(client, oid, jsid)
            if result and not result.startswith('[error]'):
                print('\n--- ' + oid + ' ---')
                print(result[:1500])
            elif result:
                print('X ' + oid + ' -> ' + result[:100])
            else:
                print('X ' + oid + ' -> no response')
        except Exception as e:
            print('! ' + oid + ' -> connection error, re-logging in...')
            # Re-login after connection drop
            jsid, key, iv = login(client)
            print('  New JSESSIONID:', jsid)
