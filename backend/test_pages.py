...
with httpx.Client(verify=False, timeout=15) as client:
    ...
    for path in ['/main/status.htm', '/main/wireless.xml', '/main/wireless.htm']:
        resp = client.get('http://192.168.0.1' + path, headers={
            'X-Requested-With': 'XMLHttpRequest', 'Referer': 'http://192.168.0.1/',
            'Cookie': 'JSESSIONID=' + jsid
        })
        if resp.status_code == 200:
            print('\n' + path + ': len=' + str(len(resp.text)))
            print(resp.text[:300])
            if len(resp.text) > 300:
                print('...')
                print(resp.text[300:600])
        else:
            print(path + ': HTTP ' + str(resp.status_code))
